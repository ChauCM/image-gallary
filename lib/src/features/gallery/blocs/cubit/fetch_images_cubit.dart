...
import 'package:image_gallary/src/features/gallery/models/network_image_dpo/network_image_dpo.dart';
...
class FetchImagesCubit extends Cubit<FetchImagesState> {
  ...
  Future<void> refresh() async {
    ...
    try {
      final images = await imagesRepository.getImages(page: 1, limit: 30);
      emit(FetchImagesState(
          status: FetchImagesStateStatus.success,
          images: images.map((e) => NetworkImageDpo.fromDto(e)).toList(),
          page: 1));
    } catch (e) {
      emit(FetchImagesState(
  ...
  Future<void> loadMore() async {
    ...
    try {
      ...
      emit(state.copyWith(
          status: FetchImagesStateStatus.success,
          images: state.images +
              images.map((e) => NetworkImageDpo.fromDto(e)).toList(),
          page: state.page + 1));
    } catch (e) {
      emit(state.copyWith(
          status: FetchImagesStateStatus.error, error: e.toString()));
